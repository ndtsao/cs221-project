#Jeff Sheng, CS 221
#Code to run RNNs on Origin of Wine Countries
...
def pad_sequences(data, max_length_description):
    ...
    for description, points in data:
        ...
        if description_len >= max_length_description:
            copy_description = copy_description[0:max_length_description] # not sure about 2nd copy description
        else:
...
def load_and_preprocess_data(debug=False):
    ...
    if debug:
        allData = [(description[_], country_dict_key_category[country[_]]) for _ in xrange(200)]

    else:
        allData = [(description[_], country_dict_key_category[country[_]]) for _ in xrange(len(description))]
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 2 and sys.argv[2] == "debug":
        debug = True
    helper, train_final_data, dev_final_data, test_final_data, train, dev, test, country_dict_key_int = load_and_preprocess_data(debug)
